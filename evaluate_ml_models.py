...
def evaluate_and_fix_models():
    ...
    # Ensure output directory for confusion matrices exists
    output_dir = "confusion_matrix"
    os.makedirs(output_dir, exist_ok=True)
    print(f"[INFO] Created directory: '{os.path.abspath(output_dir)}'")
    ...
    # -------------------------------------------------------------
    # 1. FREIGHT RATE RANDOM FOREST REGRESSOR (Model 1)
    # -------------------------------------------------------------
    ...
    if os.path.exists(freight_csv):
        ...
        # NAIVE BASELINE CHECK: does the model actually beat "predict yesterday's MA7"?
        ...
        # Discretize continuous predictions into 3 quantile tiers for Confusion Matrix
        q33, q66 = np.quantile(y_test_f, [0.33, 0.66])
        def bin_value(val):
            if val <= q33: return 0 # Low Tier
            elif val <= q66: return 1 # Medium Tier
            else: return 2 # High Tier
        ...
        png_1 = os.path.join(output_dir, "1_freight_rf_confusion_matrix.png")
        ...
        # -------------------------------------------------------------
        # 2. FREIGHT RATE GRADIENT BOOSTING REGRESSOR (Model 2)
        # -------------------------------------------------------------
        ...
        png_2 = os.path.join(output_dir, "2_freight_gb_confusion_matrix.png")
        generate_confusion_matrix_plot(
    ...
    # -------------------------------------------------------------
    # 3. VLSFO BUNKER PRICE MODEL (Model 3 - FIXED OVERFITTING)
    # -------------------------------------------------------------
    ...
    if os.path.exists(bunker_csv):
        ...
        # FIX: Apply StandardScaler + Regularized Ridge Regression to prevent overfitting
        ...
        # NAIVE BASELINE CHECK: does the bunker model beat lag1 persistence?
        ...
        # Discretize continuous predictions into 3 price tiers for Confusion Matrix
        bq33, bq66 = np.quantile(y_test_b, [0.33, 0.66])
        def bin_bunker(val):
            if val <= bq33: return 0 # Low Fuel Price
            elif val <= bq66: return 1 # Med Fuel Price
            else: return 2 # High Fuel Price
        ...
        png_3 = os.path.join(output_dir, "3_bunker_vlsfo_confusion_matrix.png")
        generate_confusion_matrix_plot(
    ...
    # -------------------------------------------------------------
    # 4. CHARTERING ACTION DECISION CLASSIFIER (Model 4 - FIXED OVERFITTING)
    # -------------------------------------------------------------
    print(f"\n[4/4] Evaluating & Fixing Vessel Chartering Action Decision Classifier...")
    df_clf = df_freight.copy()
    
    # Target signal based on MA crossover ratio (MA7 vs MA30)
    ...
    # FIX: Apply StandardScaler + tuned Random Forest (depth=3, min_samples_leaf=15) to prevent memorization
    ...
    png_4 = os.path.join(output_dir, "4_chartering_decision_confusion_matrix.png")
    generate_confusion_matrix_plot(
        cm_c, labels_map,
        "Model 4: Vessel Chartering Action Classifier - Confusion Matrix",
        f"Test Accuracy: {test_acc*100:.1f}% | Macro F1: {f1_macro:.3f} | Status: {status_c}",
        png_4
    )
    
    # Also save a copy as confusion_matrix.png in root
    generate_confusion_matrix_plot(
        cm_c, labels_map,
        "Vessel Chartering Action Classifier - Confusion Matrix",
        f"Test Accuracy: {test_acc*100:.1f}% | Macro F1: {f1_macro:.3f} | Status: {status_c}",
    ...
    # -------------------------------------------------------------
    json_path = "model_performance_evaluation.json"
    with open(json_path, "w") as f:
        json.dump(results_json, f, indent=4)
    print(f"\n[SUCCESS] Updated model evaluation JSON saved to '{os.path.abspath(json_path)}'")
    print(f"[SUCCESS] All 4 Confusion Matrix PNGs saved inside folder '{os.path.abspath(output_dir)}'")
    print("=" * 75)
